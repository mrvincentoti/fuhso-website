...
class PagesController extends AppController
{
    ...
    public function contact(){
        //debug(json_encode("TEST", JSON_PRETTY_PRINT)); exit;
        $email = new Email('default');
        if(preg_match('/http|www/i',$this->request->getData('name'))) {
            $this->Flash->error(__('We do not allow a url'));
                return $this->redirect($this->referer());
        }

        if(preg_match('/http|www/i',$this->request->getData('phone_number'))) {
            $this->Flash->error(__('We do not allow a url'));
                return $this->redirect($this->referer());
        }

        if(preg_match('/http|www/i',$this->request->getData('company_name'))) {
            $this->Flash->error(__('We do not allow a url'));
                return $this->redirect($this->referer());
        }

        if(preg_match('/http|www/i',$this->request->getData('message'))) {
            $this->Flash->error(__('We do not allow a url'));
                return $this->redirect($this->referer());
        }

        $vc = $this->request->getData('verify_contact');
        if(trim($vc) != '5'){
    ...
    public function beforeFilter(Event $event) {
        $this->Auth->allow(['requirements','contact','department','about','application','landing','principal','vc','faq']);
        if (!$this->Auth->user()) {
